...
parser.add_argument('--save', type=str, default='nonvirusdata_lr_1e-3_10000_5_3layers_kl.pt',
    ...
    with torch.no_grad():
        for i in range(0, data_source.size):
            for j in range(0,data_source[i].shape[0]-1, args.bptt):
                ...
                output,latent_space = model(data)
                sigma = torch.var(latent_space)
                mu = torch.mean(latent_space)
                output_flat = output.view(-1, ntokens)
                total_loss += len(data) * criterion(output_flat, targets).item() + len(data) * (mu**2 + sigma**2 - torch.log(sigma) - 1/2).sum()
...
def train(writer):
    ...
    for batch, i in enumerate(range(0, train_data.size)):
        for j in range(0,train_data[i].shape[0]-1, args.bptt):
            ...
            sigma = torch.var(latent_space)
            mu = torch.mean(latent_space)
            loss = criterion(output, targets) + (mu**2 + sigma**2 - torch.log(sigma) - 1/2).sum()
            loss.backward()
...
lr = args.lr
best_val_loss = None
writer = SummaryWriter(logdir="../checkpoints/nonvirusdata_lr_1e-3_10000_5_3layers_kl")
